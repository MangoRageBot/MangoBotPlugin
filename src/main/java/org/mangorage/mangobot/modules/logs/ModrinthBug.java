	...
	public void analyse(String str, Message message) {
		...
		if (lines.length > 0) {
			...
			if (last.contains("Invalid registry value type detected for PerfOS counters") && str.contains("com.modrinth.theseus")) {
				message.reply("This is a common issue on Modrinth Theseus. Modrinth's launcher has been known to be problematic in some cases with Forge. If you need to download a Modrinth format modpack you can use a more reliable launcher which supports it format.")
